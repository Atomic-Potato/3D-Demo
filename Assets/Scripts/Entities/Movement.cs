...
public class Movement : MonoBehaviour{
    ...
    void Update(){
        ...
        Vector3 cameraRight = _rotationTransform.right;
        Debug.Log(cameraRight);
        _moveInput = new Vector2(
            -cameraRight.z * Input.GetAxis("Horizontal"), 
            cameraRight.x * Input.GetAxis("Vertical")).normalized;
    ...
    void Move(){
        ...
        // if(_moveInput == Vector2.zero){ // No movement applied
        ...
        // }
        velocity = runInput ? 
            runSpeed * new Vector3(_moveInput.x, rigidBody.velocity.y, _moveInput.y) : 
            speed * new Vector3(_moveInput.x, rigidBody.velocity.y, _moveInput.y) ;
        rigidBody.velocity = velocity;
    }

    Vector3 Accelerate(Vector3 current, float maxSpeed, float time, bool onSlope = false){
        current.x = Mathf.SmoothDamp(current.x, maxSpeed * _moveInput.x, ref refVelocityX, time);
        current.z = Mathf.SmoothDamp(current.z, maxSpeed * _moveInput.y, ref refVelocityZ, time);
        if(onSlope)
    ...
    void SetMovementDirectionToInput(){
        movementDirectionOnGround.x = _moveInput.x;
        movementDirectionOnGround.z = _moveInput.y;
    }
    #endregion
